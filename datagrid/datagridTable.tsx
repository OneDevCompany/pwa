...
function renderRows(props: DatagridTableProps) {
  ...
  const keysConfiguredInTableColumnsProp = pluck<'key', string>(
    'key',
    tableColumns,
  )

  const columnsStyles = tableColumns.reduce(
    (pair, column) => ({
      ...pair,
...
function renderBodyCells(
  ...
  return keysConfiguredInTableColumnsProp.map((key) => (
    <TableCell key={key} style={getCellStyle(columnsStyles[key])}>
      {item[key]}
    </TableCell>
  ))
}
...
const handleOrderingChange =
  (clickedHeaderKey: string, props: DatagridTableProps) => () => {
